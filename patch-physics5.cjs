...
// 1. Изменяем zoneVisualRadius чтобы пузыри были меньше (особенно огромные)
code = code.replace(
  /const contentR = maxNodeR \* 3\.2 \+ Math\.sqrt\(sumNodeR\) \* 5\.2 \+ byCount \+ byShield;/g,
  'const contentR = maxNodeR * 2.0 + Math.sqrt(sumNodeR) * 2.5 + byCount * 0.5 + byShield * 0.5;'
);

// 2. В nodes loop ослабляем пружины и усиливаем расталкивание
...
const newNodesLoop = `for (let s = 0; s < 500; s++) {
    for (let i = 0; i < n; i++) {
      ...
      for (let j = i + 1; j < n; j++) {
        ...
        // Расталкивание по Катющику (увеличиваем G чтобы узлы заполняли пузырь рыхло)
        const G = 15.0; // Было 5.0
        let accI = G * massJ / (surfaceDist * surfaceDist);
        let accJ = G * massI / (surfaceDist * surfaceDist);

        if (surfaceDist < minSurfaceGap) {
          const boost = (minSurfaceGap - surfaceDist) * 3.0;
          accI += boost;
          accJ += boost;
        }
      ...
      // Ослабляем внешнее давление чтобы узлы могли разлететься внутри
      const G_ext = 4.0; // Было 8.0
      let extPressureAcc = G_ext / (boundarySurfaceDist * boundarySurfaceDist);
      
      extPressureAcc = Math.min(3.0, extPressureAcc);
      ...
      for (const depId of deps) {
        ...
        if (j !== -1 && getZid(nodeI) === getZid(nodes[j])) {
          ...
          const restSurfaceGap = minSurfaceGap * 2.0; // Увеличиваем целевую длину резинки
          
          // Закон Гука. Ослабляем пружины. a = k * dx / m
          const k = 0.5; // Было 2.0
          let springAccI = k * (surfaceDist - restSurfaceGap) / massI;
          let springAccJ = k * (surfaceDist - restSurfaceGap) / massJ;
          
          springAccI = Math.min(3.0, Math.max(-3.0, springAccI));
          springAccJ = Math.min(3.0, Math.max(-3.0, springAccJ));

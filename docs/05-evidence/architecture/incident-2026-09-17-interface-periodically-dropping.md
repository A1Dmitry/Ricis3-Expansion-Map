...
* Самостоятельный фактор, не связанный с коммитом: стартовый payload уже сейчас **~2.5 МБ JS в 25 запросах** (в dev-режиме через Vite-мидлварь — 200+ модульных запросов, см. инцидент 2026-09-16 §6: «208/208 запросов → 200»). В проксированном превью это существенная часть времени до первой отрисовки и дополнительная причина «интерфейс долго не появляется».
  > **Поправка от 2026-09-17, после pull `5748c37` (см. §12.3):** измерение верно, интерпретация была неполной. Те ~2.5 МБ в стартовом payload порождала **сама конфигурация** `rolldownOptions.codeSplitting` (группы `app`/`vendor` с `maxSize: 450_000`) — она затягивала ленивые маршруты в начальную загрузку. После её удаления стартовый payload составил **663.44 KiB в 13 ассетах**, то есть фактор устранён, а не «осознан как данность».
...
## 12. Состояние после pull `main` (2026-09-17, `5748c37`, версия 0.4.202)

По указанию владельца выполнен pull: `origin/main` влит в ветку разбора merge-коммитом `ee2514f` (история не переписывалась, force-push не применялся). Пришли коммиты `d50a883 fix: synchronize npm lockfile with Vitest 5 (#63)` и `5748c37 build: bump version to 0.4.202 and refine terminology`. Всё ниже — фактические прогоны на получившемся HEAD.

### 12.1 Что починено (измерено)

| Проблема из §11 | Статус после pull | Факт |
| :-- | :-- | :-- |
| §11.2 `npm ci` → EUSAGE (рассинхрон lockfile) | **УСТРАНЕНО** | `npm ci` → rc=0 (204 пакета верхнего уровня); `package-lock.json` синхронизирован с `vitest@5` |
| §11.2 `npm install --no-package-lock` → ERESOLVE | не проверялось повторно | обходной путь больше не нужен: штатная установка работает |
| §11.2 `security:check` → 4 moderate | **УСТРАНЕНО** | `npm audit --audit-level=moderate` → `found 0 vulnerabilities`, rc=0 |
| §11.4 красный `App.routeTopology` | **УСТРАНЕНО** | в `src/App.tsx` возвращён `const Map3D = lazyNamedComponent(() => import('./ui/Map3D'), 'Map3D')` |
| §11.3 красный `previewHealthCheck` | **УСТРАНЕНО** | литерал `allowedHosts: true` заменён на `expect(resolveDevAllowedHosts(undefined)).toBe(true)` + проверку, что `server.ts` подключает общую политику |
| §11.4 дрейф `BOARD.md` | **УСТРАНЕНО** | `npm run tps:board:check` → rc=0 |
| §11.4 3 failed / 2102 passed | **УСТРАНЕНО** | `npx vitest run` → **265 файлов / 2105 тестов, все зелёные** |

Полный набор гейтов `pr-verify.yml` на получившемся HEAD:

```console
$ npm ci                     → rc=0        $ npm run lint           → rc=0
$ npm run security:check     → rc=0        $ npm run release:check  → rc=0
$ npm run tps:gate           → rc=0        $ npm run tps:board:check→ rc=0
$ npm run tps:digest:check   → rc=0        $ npx vitest run         → 2105/2105
$ GITHUB_PAGES=true npm run generate:node-entries && npx vite build → rc=0, предупреждений 0
$ git status --short         → пусто (гейты дерево не пачкают)
```

### 12.2 Что НЕ починено: находки A, B, C2, C3, D остаются в силе

```console
$ git diff --name-only d8b414e HEAD -- server.ts server/
(пусто)
```

`server.ts` и весь каталог `server/` (включая `ricisCoreSupervisor.ts`) не менялись, поэтому:

* **A** — `const PORT = 3000` и `app.listen(PORT, "0.0.0.0", () => {…})`, игнорирующий `EADDRINUSE`, на месте: молчаливый «успех» при занятом порте воспроизводим и на 0.4.202;
* **B** — `spawnSync(dotnet, ['--version'], { timeout: 10_000 })` в пути запроса на месте: заморозка event loop и `ECONNRESET` на документе интерфейса никуда не делись;
* **C2/C3** — потребитель флага `degraded` в `src` не появился, `AbortSignal` в `callAIWithFallback` не добавлен;
* **D** — процесс по-прежнему открывает `*:24678` наряду с `0.0.0.0:3000`.

Отремонтированный `previewHealthCheck.test.ts` этих дефектов **не ловит и теперь**: он по-прежнему читает файлы (`readFileSync` + `toContain`) и вызывает чистую функцию политики, но не поднимает сервер, не делает HTTP-запроса, не моделирует занятый порт и не измеряет блокировку event loop. Контрмера №8 (реальная проба транспорта) остаётся в силе.

### 12.3 Новый факт: стартовый payload уменьшился в ~3.8 раза, но предупреждение о размере чанков подавлено, а не устранено

`5748c37` удалил из `vite.config.ts` блок `build.rolldownOptions.output.codeSplitting` (группы `app`/`vendor`, `maxSize: 450_000`) и заменил его на `chunkSizeWarningLimit: 1500`. Измерение тем же методом, что и в §11.5 (сумма JS-ассетов, на которые ссылается собранный `dist/index.html`; значения в **KiB = 1024 байта**, тогда как лог Vite печатает **kB = 1000 байт** — отсюда расхождение 618.88 KiB против 633.73 kB для одного и того же файла):

| Сборка | Ассетов в стартовом payload | Стартовый JS | Крупнейший в стартовом payload | Крупнейшие чанки вообще |
| :-- | :-- | :-- | :-- | :-- |
| `97583b6` (codeSplitting-группы) | 25 | 2525.13 KiB | 364.82 KiB (vendor) | 373.57 kB по логу Vite |
| `5d6d23f` (Map3D статический) | 25 | 2550.10 KiB | 364.82 KiB (vendor) | — |
| **`5748c37` (группы удалены)** | **13** | **663.44 KiB** | **618.88 KiB (`index-*.js`)** | `OrbitControls` 780 kB, `Map3D` 692 kB, `index` 620 kB (по логу Vite) |

Два следствия, которые надо держать раздельно:

1. **Пользовательский эффект положительный.** Стартовая загрузка превью стала в ~3.8 раза легче: тяжёлые `Map3D`/`OrbitControls`/`KinematicEnginePage` остались ленивыми и в начальном payload не участвуют. Прежние ~2.5 МБ были следствием конфигурации групп, а не «тяжести приложения» — см. поправку в §11.5.
2. **Возник новый документированный drift.** Крупнейшие чанки теперь превышают 500 kB (780/692/620 kB), а предупреждение сборки отсутствует (`предупреждений в логе сборки: 0`) **не** потому, что чанков >500 kB нет, а потому, что порог поднят до 1500. Это прямо противоречит зафиксированному в `ACTIVE_TASKS.md` («Продолжение 2026-09-16: P2 и F-09/F-11»): «`vite.config.ts` использует Rolldown `output.codeSplitting` … с `maxSize: 450_000`. Производственная сборка больше не выдаёт предупреждение о чанках >500 kB; крупнейший фактический chunk — 373.57 kB. **Это реальное дробление графа модулей, а не повышение `chunkSizeWarningLimit`**». Ни `ACTIVE_TASKS.md`, ни этот тезис в `main` не обновлены, поэтому заявленное больше не соответствует фактическому — тот же класс, что F-01/QA-1 (метрика вместо основания) и §4.4 инцидента 2026-09-16.

Блокером это не является (чанки ленивые, стартовый payload меньше прежнего), но требует явного решения владельца: либо вернуть реальное дробление с порогом 500 kB, либо обновить `ACTIVE_TASKS.md`/документацию так, чтобы заявленное совпадало с `vite.config.ts`.

### 12.4 Граница этого раздела

* Прогоны выполнены в песочнице, где `dotnet` отсутствует и TLS к `*.googleapis.com` закрыт, поэтому находка B проверена только на подставном медленном host'е (§3), а C1/C3 — только по коду и сетевым пробам (§4). «Все гейты зелёные» означает зелёные гейты **в этой среде**, а не подтверждение работы ядра или внешнего AI-канала.
* `AUDITOR: SELF (same-pipeline)` — раздел написан тем же пайплайном, который выполнял pull и прогоны.

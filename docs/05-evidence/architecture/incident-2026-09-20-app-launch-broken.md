...
Trailer `documentation-restudy: <ключ>` в исправляющем коммите здесь **не применим**: он требуется
по §3 G2, когда нарушитель атрибутируем; у сломавших коммитов ключа нет (вердикт `REJECT`), поэтому
требование адресовано вкладу как таковому, а не конкретному ключу.
...
* CI PR #89 (ветка `arena/01a0bf4a-…`): run 35519230609 — шаги 1–9 зелёные (включая «Onboarding gate»), шаг 10 красный по аттестации (§5.1); после смены ключа run 35524888840 — шаги 1–9 зелёные, шаг 10 красный из-за синтетического merge-коммита (§4a) → устранено привязкой checkout к head-sha PR; **финальный run 35525020961 (head `64cafac`) — все 12 шагов зелёные**, включая шаг 10 «Executor traceability gate», шаг 11 «Run test suite» и шаг 12 «Build static application without deployment» (2m38s). Локальные ворота по ветке — `чисто (проверено коммитов: 3, база origin/main)`.

### 7.1 Закрытие (развёртывание восстановлено)

* PR #89 слит в `main` обычным merge-коммитом `c6701ec1a5a7ed6c2e8733ba187c41f4861d5e46`
  (2026-09-20T17:16:47Z); заголовок merge-коммита несёт ключ `cdd5cf6c…`, зарегистрированный в дереве
  на этот момент. Squash не применялся — заголовки §2 коммитов ветки сохранены в истории `main`;
* **Deploy to GitHub Pages, run 35525358878** — success (3m14s): сборка, полный `npm test` и публикация
  прошли; страница `https://a1dmitry.github.io/Ricis3-Expansion-Map/` обновлена. Это первая зелёная
  публикация после `ab14628` (2026-09-20T01:17Z) — приложение снова запускается и раздаётся;
* прямой `curl` к `github.io` из песочницы недоступен (SSL-туннель закрыт), поэтому факт проверен по
  коду завершения workflow и по артефакту, а не по содержимому страницы.

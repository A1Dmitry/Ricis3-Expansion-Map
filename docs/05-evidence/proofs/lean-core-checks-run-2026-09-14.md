...
| **F-08** | MEDIUM | `ricis-seed-expansion-a11`: **дефект моей собственной подстановки**. Замена Mathlib-леммы `List.mem_of_mem_append_left` на `List.mem_append.mpr (Or.inl hr)` оказалась неприменима: `104:4 error: Type mismatch … has type r ∈ s.rules ++ ?m.77 but is expected to have type r ∈ a✝¹.rules` (после `induction` цель уже не содержит `++`), далее `105:4 Tactic `split` failed`, `156:63`/`165:50`/`176:105 unsolved goals`; 3 из 6 теорем получили `sorryAx`. | Зафиксировано как неудачный ремонт (ANTI-TUKHTA LAW: собственный отказ не замалчивается). Требуется ядровый шаг, соответствующий цели после `induction`, и усиление трёх доказательств; до этого производная не считается ремонтом. **РЕШЕНО 2026-09-15** (локальный прогон ядра 4.33.1, тот же commit, что в CI: отказ воспроизведён дословно, первопричина — семантика ядрового `split`, построено ядровое доказательство; финальная производная exit 0, 6/6 propext, без sorryAx; A11 исключена из ciPolicy; evidence: `docs/05-evidence/proofs/lean-core-checks-local-run-2026-09-15.md`; CI-репрогон после восстановления подключения к GitHub). |
...
## 10. Addendum 2026-09-15 — Mathlib-путь ядровой проверки и аудит нового внешнего артефакта

**Версия приложения:** 0.4.190. **AUDITOR: SELF (same-pipeline)** — аудит и генератор выполнены
той же агентной сессией; независимым является только прогон ядра (Lean 4.33.x, workflow
`lean-artifact-kernel-check.yml`), факт которого фиксируется в реестре и этом документе.

### 10.1 Нарушение F-12: основание `REQUIRES_CORE_LEAN` было недостоверным

В 0.4.189 14 артефактов с `import Mathlib` числились `REQUIRES_CORE_LEAN` с формулировкой
«No pinned prebuilt Mathlib fits a standard runner». Утверждение верно только для **сборки**
Mathlib из исходников. Готовые oleans закреплённой ревизии (`lake exe cache get`) на стандартный
runner влезают, поэтому отсутствие prebuilt-Mathlib перестаёт быть основанием, а сам статус
начинает маскировать невыполненную проверку — то есть метрика («прогон не запускался») выдаётся
за состояние объекта («проверить нельзя»).

Ремонт (0.4.190):

* job `mathlib-kernel-check` в `lean-artifact-kernel-check.yml`: тулчейн из `lean-toolchain`
  закреплённой ревизии Mathlib (`leanprover/lean4:v4.33.0`, Mathlib @
  `6f1ef4e5dd604a435bddba4747b13970cd65d2a1` — новейшая ревизия mathlib4 на линии 4.33.x;
  2026-09-15 master перешёл на `v4.34.0`), `lake update` + `lake exe cache get`,
  затем `lake env lean <файл>`;
* скрипт `scripts/mathlibKernelCheck.sh` и генератор
  `scripts/generateLeanMathlibChecks.ts` (производная = **исходник байт-в-байт** + эпилог
  `#print axioms`; подстановок нет вообще);
* отдельный список ожидаемых отказов `ciPolicy.mathlibExpectedFailures` (тот же закон, что и для
  core-check: единственный источник — реестр; `sorryAx` в скопилированном файле рвёт прогон всегда);
* стражи `tools/leanMathlibChecks.test.ts` (9 тестов: детерминизм, байтовое равенство тела,
  запрет подстановок, реальность целей `#print axioms`, целостность manifest, запрет повышения
  статуса без факта прогона, политика workflow/скрипта).
* Граница: механизм сам по себе **ничего не повышает** — прогон запускается только для файлов из
  явного allowlist `MATHLIB_ARTIFACTS`; остальные Mathlib-артефакты остаются `REQUIRES_CORE_LEAN`
  до собственного прогона (отдельная задача, решение владельца).

### 10.2 Новый внешний артефакт: `ricis-general-resolution.lean` (F-09, F-10, F-11)

Владелец передал Lean-исходник «ОБЩАЯ теорема разрешения комплексных сингулярностей
(ФИНАЛЬНАЯ ВЕРСИЯ 2)» с инструкцией «интегрировать в проект». По AGENTS.md §7 исходник сохранён
**байт-в-байт** (`artifacts/proofs/ricis-general-resolution.lean`,
sha256 `e92ebe52a85af838205a6bdb950ff4517f959cbfc4f38c1d0e7c4cb65cc35db3`), статус —
`REQUIRES_CORE_LEAN`; метаданные присоединены снаружи (`ricis-general-resolution.json`).
Интеграция выполнена как **верификация заявленного результата**, а не как молчаливое принятие
статуса: аудит содержимого зафиксирован как находки реестра.

* **F-09 (HIGH) — метрика вместо исходной задачи.** `ricis_eval_general` объявлена ровно тем
  выражением, которое утверждает `ricis_general_resolution`; доказательство — `dsimp` +
  `by_cases` + `omega`/`simp`. `ax_A4_general`, `ax_SP1_general` и `ricis_reduce` не участвуют ни
  в одном доказательстве. Заголовок «ОБЩАЯ теорема разрешения сингулярностей» содержимым файла не
  подтверждается.
* **F-10 (HIGH) — необъявленная граница доверия.** Две `axiom`-декларации — доверенные контракты,
  а не доказательства; имя `ricis_equals_classical_limit` обещает классический предел, которого в
  файле нет (P1 RICIS запрещает пределы внутри `Resolve_RICIS`). Контракты зафиксированы в
  метаданных (`declaredAxioms`) и печатаются эпилогом прогона (`#print axioms`).
* **F-11 (MEDIUM) — разрыв «редукция ↔ вычисление».** `ricis_reduce` (сокращение
  `(z-a)^min(m,n)`) не используется; ни одна теорема не связывает редукцию со значением;
  сингулярность `N a = 0 ∧ D a = 0` не предъявлена — объекта разрешения в артефакте нет.

**Статус артефакта:** `REQUIRES_CORE_LEAN` (не `LEAN_VERIFIED`, не `TRUSTED_AXIOM`). Прогон
зарегистрирован в `pendingKernelRun` со `expectedOutcome: UNKNOWN`; результат прогона вносится
в этот документ и реестр отдельным коммитом. Статический аудит (UNVERIFIED_PREDICTION) —
в [`ricis-general-resolution-claim-audit-2026-09-15.md`](ricis-general-resolution-claim-audit-2026-09-15.md).

### 10.3 Что осталось (честно)

1. Фактический прогон пилотной Mathlib-цели (job `mathlib-kernel-check`); при красном исходе —
   запись в реестр с дословной первопричиной, при зелёном — решение владельца о повышении статуса.
2. Список `MATHLIB_ARTIFACTS` для остальных 13 Mathlib-артефактов — отдельное решение владельца
   (порядок и бюджет прогонов).
3. F-08 (ядровой ремонт A11), L9 (узел `registry-120`), F-05 (формулировки узлов карты) —
...
## 12. Addendum 2026-09-15 (вечер) — сверка с `main`, восстановление workflow и находка F-14

**Контекст.** Ветка PR #40 разошлась с `main`: `main` (коммиты `ff954f7` → `142d744` → `4560a7f`,
прямые пуши владельца, самый свежий — 2026-09-15T09:39Z) ушёл вперёд и **потерял**
`.github/workflows/lean-artifact-kernel-check.yml`: файл исчез при разрешении merge-конфликта в
коммите `142d744` («added lean» — merge со устаревшей ветки). Ни один коммит `--diff-filter=D`
его удаления не показывает именно потому, что удаление произошло **внутри merge-разрешения** —
тот же класс инцидента, что F-07 (утрата `deriving`-клаузы SP5 при мерже PR #37).

**Что сделано в этой ветке:**

1. `origin/main` влит в ветку (`git merge origin/main`). Конфликт ровно один — modify/delete
   указанного workflow. Разрешён **сохранением файла** (обе job-ы: `kernel-check` и
   `mathlib-kernel-check`): без workflow все статусы `LEAN_VERIFIED` становятся безосновательными
   по §7, а evidence-канал исчезает. Восстановление зафиксировано комментарием в самом файле.
2. Ветка приняла из `main` исправления seed-домена (`0d23090`: гейт `SEMANTIC_RULE_VERIFIED` в
   документе v8.0 + тест duplicate-axiom) — именно из-за их отсутствия локальный прогон на базовом
   коммите давал 2 красных seed-теста, тогда как CI на merge-рефе PR #40 был зелёным.
3. **F-14 (HIGH, новая находка):** артефакт `artifacts/proofs/ricis-yang-mills.lean`, добавленный
   в `main` (`4560a7f`), объявляет в шапке «Статус: 100% компилируется, 0% sorry», но импортирует
   `Mathlib.Basic.Real.Basic`, **которого нет** в закреплённой ревизии Mathlib
   (`6f1ef4e5dd604a435bddba4747b13970cd65d2a1`): отсутствуют `Mathlib/Basic.lean`,
   `Mathlib/Basic/Real/Basic.lean`, `Mathlib/Real/Basic.lean`; существуют `Mathlib/Tactic/Ring.lean`
   и `Mathlib/Data/Real/Basic.lean` (проверено GitHub contents API по дереву этой ревизии).
   Метаданных, `trustStatus` и записи в прогонах у артефакта нет. Артефакт внесён как
   `REQUIRES_CORE_LEAN` + `pendingKernelRun` (expectedOutcome `UNKNOWN`); в allowlist
   `MATHLIB_ARTIFACTS` **не** внесён: запуск цели с несуществующим целевым модулем не даёт evidence
   о теоремах, а расширение allowlist/регистрация ожидаемого отказа — решение владельца. Байты
   артефакта не изменялись (§7); ремонт импорта возможен только как новая версия доказательства.

**Граница:** F-14 — факт о дереве библиотеки и о шапке артефакта, а не приговор содержанию теорем;
он фиксирует, что заявление «100% компилируется» не подтверждено и в предоставленном виде не
выполнимо. Никакой статус артефакту не присвоен.

## 13. Addendum 2026-09-16 — прогон run 35145205870 (P12) и находка F-15

Контекст: цикл P12 (ACTIVE_TASKS §0, пункт 1) расширил явное покрытие ядровых прогонов на три
артефакта — `RicisAgiTarget.lean`, `jacobian-counterexample-full.lean` (allowlist
`MATHLIB_ARTIFACTS`) и `Schwarzschild_GeometricBridge.lean` (core-check путь: тело не использует
символы Mathlib, три неиспользуемые строки импорта удалены в производной).

### 13.1 Факты прогона (run 35145205870, PR #59)

* **`Schwarzschild_GeometricBridge.core-check.lean`** (джоба `kernel-check`, Lean 4.33.1):
  **exit 0, ошибок 0, sorryAx отсутствует**; `#print axioms`: 7 теорем не зависят ни от каких
  аксиом, 4 (`eval_product_ok`, `eval_product_none_foreign`, `eval_ratio_ok`,
  `eval_ratio_none_foreign`) зависят только от `propext`. Статический вывод «тело не использует
  Mathlib» стал фактом прогона. Артефактный уровень — `LEAN_VERIFIED`; физические заявления
  (регулярность метрики, устранение сингулярности ОТО) не заявлены и не проверялись — узел карты
  остаётся `REQUIRES_CORE_LEAN`.
* **`RicisAgiTarget.lean`** (джоба `mathlib-kernel-check`): исходник как предоставлен — **exit 0**,
  производная — **exit 0**, sorryAx отсутствует. Доверенных контрактов нет: все три теоремы зависят
  только от стандартных аксиом (`propext`, `Classical.choice`, `Quot.sound`). Артефактный уровень —
  `LEAN_VERIFIED`, уровень заявления — `STRUCTURALLY_VALIDATED` (ничего эмпирического об AGI).
* **`ricis-general-resolution-v3.lean`**: исходник — **exit 0**, производная — **exit 0**, sorryAx
  отсутствует. Факт ядра по разрыву F-09/F-11 на артефактном уровне: сингулярность `N a = 0 ∧ D a = 0`
  предъявлена (`singularity_is_zero_over_zero`), `ricis_reduce` в центре равен `ricis_eval_general`,
  а `ax_A4_general`/`ax_SP1_general` — ФАКТИЧЕСКИЕ зависимости `ricis_resolution_bridge` по
  `#print axioms`; контракты остаются объявленными (не доказанными). Пределы не используются (P1).
* **`ricis-general-resolution.lean`** (повторно): исходник и производная — снова **exit 0**, sorryAx
  отсутствует; факт первого прогона (34950902412) подтверждён.
* **`jacobian-counterexample-full.lean`**: исходник как предоставлен — **exit 0** (все 10 теорем
  приняты, `#print axioms` напечатан, включая зависимость `trusted_contract_no_left_inverse` от
  доверенного контракта). Производная — **exit 1** по дефекту генератора эпилога (см. 13.2).

### 13.2 Находка F-15 (MEDIUM) — эпилог не квалифицировал аксиому внутри неймспейса

Дословная ошибка производной:
`artifacts/proofs/mathlib-checks/jacobian-counterexample-full.mathlib-check.lean:162:14:
error(lean.unknownIdentifier): Unknown constant trusted_full_jacobian_contract`.

Первопричина: `collectDeclaredAxioms` собирал имена `axiom`-деклараций без отслеживания
неймспейсов, тогда как эпилог стоит после всех `end …`; пилотные артефакты объявляли аксиомы
только на верхнем уровне, поэтому класс дефекта не был покрыт. Страж целей эпилога сверял их
против того же сборщика (самосогласованно, а не независимо) и проглатывал неразрешимое имя.
Это дефект генератора, а НЕ дефект доказательства: исходник скомпилирован ядром как предоставлен.

Ремонт (этот цикл): единый скоуп-трекер `collectScopedNames` для теорем и аксиом
(`scripts/generateLeanCoreChecks.ts`), производная перегенерирована (квалифицированное имя
`JacobianCounterexample.trusted_full_jacobian_contract`), независимый структурный страж
квалификации добавлен в `tools/leanMathlibChecks.test.ts` (A-0011). Двойной прогон исправленной
производной ожидает повторного запуска джобы; до его факта статус артефакта не повышен
(`REQUIRES_CORE_LEAN`, `PENDING_KERNEL_RUN`).

### 13.3 Граница доверия аддендума

Записаны только факты из комментария PR #59 (сохранён как

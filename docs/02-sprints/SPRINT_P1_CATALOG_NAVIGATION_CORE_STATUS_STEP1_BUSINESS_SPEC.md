...
| 3 | Написать Vitest QA до реализации. | Тесты сначала red: generic new-node-plus-edge сценарий, link-only `core-agi-target → real-catalog-98`, exact repeat idempotent, invalid edges rejected, no proof/state mutation. | Утверждённый Step 3 QA suite. |
...
| 5 | Подготовить link-only `RICIS.MapStatePatch` JSON с одной edge и без nodePatch. | Validation сообщает `patch_merge`; apply result на существующем узле: `createdNodeCount: 0`, `createdEdgeCount: 1`, `proofsAttachedCount: 0`; existing type/state не изменены. | Безопасный пользовательский JSON для восстановления связи. |

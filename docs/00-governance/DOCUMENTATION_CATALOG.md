...
|---|---|---|---|
| `docs/00-governance/` | Strict Development Rules, Work Patterns (P-01..P-12), RCVAP Autonomous Anti-Tukhta Agile Protocol, RICIS Proof Orchestration Template (2-layer pipeline), RICIS Immutability Manifest (Core Invariance), contribution/security policies, documentation catalog. | Высокая (Абсолютная для Манифеста Неизменности). | Читать до проектирования или изменения public/security/release boundary. |
...
| `docs/02-sprints/` | Active and planned sprint specifications, acceptance criteria, implementation dependencies. **Восстановленный архив** спринтовой документации линии до сквоша main (состояние 2026-08-27); текущее планирование живёт в `ACTIVE_TASKS.md` (корень). | Контекстно высокая (историческая). | Сопоставлять со статусом и датой; закрытый sprint не является current runtime fact. |
| `docs/03-quality/` | QA baselines, findings, regression/release evidence summaries. **Восстановленный архив** QA-документации линии до сквоша main (состояние 2026-08-27). | Фактическая на дату проверки (историческая). | Не переносить исторический PASS на изменённый codebase. |
...
| `docs/06-canonical-template/` | Compressed application blueprint и reusable checklists. **Восстановленный архив:** `STRICT_DEVELOPMENT_RULES.md` линии до сквоша main (состояние 2026-08-27). | Шаблонный нормативный reference (исторический). | Использовать при создании нового приложения; адаптировать к domain, не копировать security claims без проверки. |

...
* **Карта**: `initialMap.ts` — 54 узла. `core-agi-target` — `resolved` с **реальным DOI 10.5281/zenodo.22225762**; вся frontier детей открыта (`med-diagnostics`, `pharm-design`, `phys-unified`, `econ-value`, `ethic-alignment`, `ai-authorship-provenance`, `ricis-chatbot-monetization`).

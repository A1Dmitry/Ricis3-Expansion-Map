...
[5]: `Ricis.WebApi` (файл `Ricis.Core/Ricis.WebApi/Program.cs` находился во внешнем репозитории вне дерева, ссылка приведена как текст) — current bounded minimal API conventions.

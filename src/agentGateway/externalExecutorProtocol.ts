...
EOF
cat > /home/user/Ricis3-Expansion-Map/src/agentGateway/externalExecutorProtocol.test.ts <<'EOF'
import { describe, expect, it } from 'vitest';
import {
  assessExternalExecutorEntry,
  gateExternalExecutorSubmission,
  RICIS_NORMATIVE_DOCUMENT,
} from './externalExecutorProtocol';
...
describe('external executor entry and solution gate', () => {
  it('accepts a compliance-first executor without requiring extra reading', () => {
  ...
  it('rejects a non-compliant result regardless of executor strategy', () => {
    const result = gateExternalExecutorSubmission({ ...base, appliedRules: [], resultValid: false }, ['L1']);
    expect(result.accepted).toBe(false);
  });

  it('rejects protected redefinition and does not mutate the normative boundary', () => {
    ...
    const stillAccepted = gateExternalExecutorSubmission({ ...base, appliedRules: ['SP4'], resultValid: true }, ['SP4']);
    expect(stillAccepted.accepted).toBe(true);
  });

  it('routes unknown rules to the normative source rather than inventing a replacement', () => {
  ...
  it('provides a minimal entry handshake for an unknown executor', () => {

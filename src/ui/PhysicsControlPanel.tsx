...
const CustomRangeSlider = memo(function CustomRangeSlider({
  ...
  const handleNumberChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = parseFloat(e.target.value);
    onChangeValue(prop, isNaN(val) ? 0 : val);
  };
      ...
      >
        ...
        <div className="accordion-summary mt-2 pt-1.5 border-t border-neutral-800/40 flex flex-wrap gap-1.5 text-xs font-mono text-emerald-300 w-full">
          <span className="bg-neutral-900 border border-neutral-800 px-2 py-0.5 rounded text-slate-300">
            G Зон: <strong className="text-emerald-400">{workingParams.zoneG}</strong>
          </span>
          <span className="bg-neutral-900 border border-neutral-800 px-2 py-0.5 rounded text-slate-300">
            G Узлов: <strong className="text-emerald-400">{workingParams.nodeG}</strong>
          </span>
          <span className="bg-neutral-900 border border-neutral-800 px-2 py-0.5 rounded text-slate-300">
            k: <strong className="text-emerald-400">{workingParams.springK}</strong>
          </span>
        </div>
      </label>

      <div className="accordion-content">
        <div className="accordion-inner p-3 text-xs text-slate-200 border-t border-neutral-800/60 space-y-3 bg-neutral-950/40">
          ...
          <div className="bg-neutral-900/80 border border-neutral-800 rounded-lg p-3 space-y-2.5 shadow-sm">
            ...
            <CustomRangeSlider
              label="Отталкивание масс (G)"
              prop="zoneG"
              min={0}
              max={100}
              step={1}
              value={workingParams.zoneG}
              onChangeValue={handleSliderChange}
              onStart={startInteraction}
              onEnd={endInteraction}
            />
            <CustomRangeSlider
              label="Давление среды (G_ext)"
              prop="zoneGExt"
              min={0}
              max={100}
              step={1}
              value={workingParams.zoneGExt}
              onChangeValue={handleSliderChange}
              onStart={startInteraction}
              onEnd={endInteraction}
            />
            <CustomRangeSlider
              label="Зазор между зонами"
              prop="zoneSurfaceGap"
              min={0}
              max={100}
              step={5}
              value={workingParams.zoneSurfaceGap}
              onChangeValue={handleSliderChange}
              onStart={startInteraction}
              onEnd={endInteraction}
            />
          ...
          <div className="bg-neutral-900/80 border border-neutral-800 rounded-lg p-3 space-y-2.5 shadow-sm">
            ...
            <CustomRangeSlider
              ...
              max={50}
              step={1}
              value={workingParams.nodeG}
              onChangeValue={handleSliderChange}
              onStart={startInteraction}
              onEnd={endInteraction}
            />
            <CustomRangeSlider
              label="Внешнее давление (G_ext)"
              prop="nodeGExt"
              min={0}
              max={20}
              step={0.5}
              value={workingParams.nodeGExt}
              onChangeValue={handleSliderChange}
              onStart={startInteraction}
              onEnd={endInteraction}
            />
            <CustomRangeSlider
              label="Жесткость пружин (k)"
              prop="springK"
              min={0}
              max={5}
              step={0.1}
              value={workingParams.springK}
              onChangeValue={handleSliderChange}
              onStart={startInteraction}
              onEnd={endInteraction}
            />
            <CustomRangeSlider
              label="Целевая длина пружин"
              prop="springRestGapMult"
              min={1}
              max={10}
              step={0.5}
              value={workingParams.springRestGapMult}
              onChangeValue={handleSliderChange}
              onStart={startInteraction}
              onEnd={endInteraction}
            />
            <CustomRangeSlider
              label="Мин. зазор (узлы)"
              prop="minNodeSurfaceGap"
              min={1}
              max={20}
              step={1}
              value={workingParams.minNodeSurfaceGap}
              onChangeValue={handleSliderChange}
              onStart={startInteraction}
              onEnd={endInteraction}
            />
            <CustomRangeSlider
              ...
              max={1}
              step={0.05}
              value={workingParams.edgeOpacity}
              onChangeValue={handleSliderChange}
              onStart={startInteraction}
              onEnd={endInteraction}
            />

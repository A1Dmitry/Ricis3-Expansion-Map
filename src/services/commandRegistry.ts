...
export const APP_COMMANDS: readonly AppCommand[] = [
  ...
  {
    id: 'nav.proof-logs',
    label: 'Логи Ядра Lean 4',
    tooltip: 'Просмотр результатов ядровой проверки Lean 4 для текущего узла',
    ...
    group: 'workspace',
    isEnabled: () => true,
    isActive: ctx => ctx.activeApplet === 'proof-logs',
    execute: ctx => ctx.onSelectApplet('proof-logs'),
  },
  {
    id: 'nav.p-vs-np',
    label: 'P vs NP Факторизация',
    tooltip: 'Решение задачи факторизации через непрерывные сингулярности RICIS',
    iconName: 'Cpu',
    category: 'navigation',
    appletScope: 'all',
    shortcut: 'Alt+F',
    group: 'workspace',
    isEnabled: () => true,
    isActive: ctx => ctx.activeApplet === 'p-vs-np',
    execute: ctx => ctx.onSelectApplet('p-vs-np'),
  },
  {
    id: 'nav.qa-tests',

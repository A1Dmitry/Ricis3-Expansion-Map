...
Требуется Node.js `22.22.2` или новее в пределах ветки 22 и npm `12.0.2` или новее. Нельзя устанавливать устаревшие версии зависимостей или обходить конфликты через `--force` и `--legacy-peer-deps`. `package.json` — единый источник версии; `package-lock.json` и `src/version.ts` должны оставаться синхронизированными. В репозитории используется только npm lockfile.

...
Правая карточка задачи на desktop получила два состояния: полноценная docked-панель и узкая вертикальная rail-полоса. В rail-режиме 3D-сцена расширяется на освободившееся место; кнопка со стрелкой возвращает панель. При отсутствии выбранной задачи третья колонка полностью исчезает.

### v0.4.18 — Desktop right-docked task accordion

Карточка выбранной задачи на desktop вынесена из overlay в отдельную правую grid-панель, пристыкованную к краю окна. Содержимое раскрывается единым аккордеоном, а лишняя внешняя рамка карточки удалена.
...
На touch-first экранах используется независимый mobile shell: portrait показывает 3D-сцену над контекстной областью, landscape располагает сцену рядом с control column. Hamburger открывает menu-first навигацию к поиску, карточке узла и настройкам; стандартный Back возвращает к предыдущему mobile экрану. Двойное касание 3D-сцены включает/выключает fullscreen, а явно включаемый Sensor Mode калибрует `DeviceOrientation` и управляет орбитой камеры без замены ручных жестов.

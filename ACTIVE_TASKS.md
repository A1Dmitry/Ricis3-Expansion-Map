...
* **Статус:** `PARTIALLY_COMPLETED` (пакеты 1 и 2 прогнаны фактически: run 34858902595 + run 34870620154; 8 из 12 целей exit 0; v79 и SP5 — ремонты выполнены, повторный прогон в этом PR; F-01: решение владельца по артефактному уровню внесено, узел-уровень — L9; **F-08 (A11) — ядровой ремонт выполнен 2026-09-15 локальным прогоном 4.33.1 (exit 0, 6/6 propext, без sorryAx), A11 исключена из ciPolicy.expectedFailures; CI-репрогон ожидается (соединение с GitHub в среде сессии потеряно)**)
...
* **Осталось (не закрыто):** (1) повторный прогон v79 (после F-06-ремонта) и SP5 (перегенерированная) в этом PR + внесение фактов в реестр — **выполнено прогоном 34891262489** (оба exit 0, без sorryAx; факты в реестре); (2) решение владельца L9: узел `registry-120` в `initialMap` (TRUSTED_AXIOM) и F-05 (формулировки узлов карты); ~~(3) F-08: ядровой ремонт A11~~ — **выполнено 2026-09-15** (см. выше; CI-репрогон — после восстановления подключения к GitHub, соединение в среде сессии потеряно); (4) `RicisAgiTarget.lean` и `jacobian-counterexample-full.lean` остаются `REQUIRES_CORE_LEAN` (реально требуют Mathlib: `ℝ`/`ℚ` + `ring`/`norm_num`).

...
* **Найденная ТУФТА (зафиксирована, не замалчивается):**
  ...
  * **F-07 (HIGH):** SP5 — в ядре не синтезируется `Decidable (a = b)` для `RExpr` (Mathlib давал его через deriving-механизмы); обе теоремы с `sorryAx`.

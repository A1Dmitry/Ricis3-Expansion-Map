...
* **Статус:** `PARTIALLY_COMPLETED` (артефакт принят §7 байт-в-байт, аудит выполнен, механизм прогона создан; фактический прогон ядра — PENDING_KERNEL_RUN)
...
* **Осталось:** (1) фактический прогон пилотной Mathlib-цели и внесение фактов в реестр; (2) решение владельца о статусе артефакта после прогона (зелёный ядро-прогон не снимает F-09/F-10/F-11 — они о классе заявленного результата); (3) расширение allowlist `MATHLIB_ARTIFACTS` на остальные Mathlib-артефакты — отдельное решение владельца; (4) содержательный ремонт — новая версия доказательства, связывающая `ricis_reduce`/A4/SP1 со значением (исходник не перезаписывается, §7).

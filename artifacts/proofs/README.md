...
| `Schwarzschild_GeometricBridge.lean` | `REQUIRES_CORE_LEAN` | Path-indexed Schwarzschild-bridge monolith with A6 computational proxy (canon L1, SP2, SP4, A6 proxy, P1 no-lim; DOI 10.5281/zenodo.22124493 workflow provenance); imports `Mathlib.Data.Int.Basic` / `Mathlib.Data.String.Basic` / `Mathlib.Tactic`, outside the `MATHLIB_ARTIFACTS` allowlist — no kernel run yet; metadata `Schwarzschild_GeometricBridge.json` pins sha256 and the scope boundary |
...
| `Schwarzschild_GeometricBridge.lean` (2026-09-15)                                | `REQUIRES_CORE_LEAN`     | Metadata JSON present with pinned sha256 and explicit scope boundary (path identity, non-collapse, A6 product/ratio proxy on Int are facts; Schwarzschild metric regularity / GR singularity removal are NOT claimed). Imports Mathlib, is outside the `MATHLIB_ARTIFACTS` allowlist, and has no kernel run — status is not promoted |

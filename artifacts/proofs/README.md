...
| `Schwarzschild_GeometricBridge.lean` | `REQUIRES_CORE_LEAN` | Path-indexed Schwarzschild-bridge monolith with A6 computational proxy (canon L1, SP2, SP4, A6 proxy, P1 no-lim; DOI 10.5281/zenodo.22124493 workflow provenance). Body uses only core-native primitives (Nat/Int/String, rfl/simp/native_decide); the three Mathlib imports are unused and are removed in the generated core-check derivative `core-checks/Schwarzschild_GeometricBridge.core-check.lean` — registered for the `kernel-check` job, `PENDING_KERNEL_RUN`; status is not promoted until the actual run. Metadata `Schwarzschild_GeometricBridge.json` pins sha256 and the scope boundary |
...
| `Schwarzschild_GeometricBridge.lean` (2026-09-15)                                | `REQUIRES_CORE_LEAN`     | Metadata JSON present with pinned sha256 and explicit scope boundary (path identity, non-collapse, A6 product/ratio proxy on Int are facts; Schwarzschild metric regularity / GR singularity removal are NOT claimed). The three `import Mathlib…` lines are unused by the body (Nat/Int/String + rfl/simp/native_decide only — established by file reading, A-0007 blocks local compilation); the generated core-check derivative is registered for the `kernel-check` job (`PENDING_KERNEL_RUN`). Status is not promoted until the actual run |
| `RicisAgiTarget.lean` (2026-09-16)                                               | `REQUIRES_CORE_LEAN`     | Body genuinely needs Mathlib (`ℝ` + `ring`); metadata `RicisAgiTarget.json` pins sha256; added to the `MATHLIB_ARTIFACTS` allowlist with a byte-identical derivative + additive `#print axioms` epilogue (`PENDING_KERNEL_RUN`). Empirical AGI claims are NOT made; status is not promoted until the actual double run |
| `jacobian-counterexample-full.lean` (2026-09-16)                                 | `REQUIRES_CORE_LEAN`     | Body genuinely needs Mathlib (`ℚ` + `ring`/`norm_num`); metadata `jacobian-counterexample-full.json` pins sha256; added to the `MATHLIB_ARTIFACTS` allowlist with a byte-identical derivative + additive `#print axioms` epilogue (`PENDING_KERNEL_RUN`). Declared `trusted_full_jacobian_contract` axiom stays a trusted contract, not a proven lemma; no claim about the classical Jacobian conjecture as a whole. Status is not promoted until the actual double run |
...
* **Allowlist extension (2026-09-16, P12):** `RicisAgiTarget.lean` (body needs `ℝ` + `ring`) and
  ...
  with byte-identical derivatives and honest `PENDING_KERNEL_RUN` records; the actual double run
  (source as provided + derivative) is produced by CI — no status is promoted by registration.
  `Schwarzschild_GeometricBridge.lean` was established (by file reading; local toolchain unavailable,
  A-0007) to have a Mathlib-free body and takes the **core-check** path instead (its three unused
  `import Mathlib…` lines are removed in the generated derivative).

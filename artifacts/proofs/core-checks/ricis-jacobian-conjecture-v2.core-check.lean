...
/-! ===== GENERATED KERNEL-CHECK EPILOGUE (additive only) =====
  ...
  Source      : artifacts/proofs/ricis-jacobian-conjecture-v2.lean
  Source hash : sha256 cc7d8d44551e4120bda8b504b3ad6cbcb99734ac3a0a1809139cff25ae20b364
  ...
  Basis       : Тело — НОВАЯ версия артефакта якобиана (F-01): опровержение утверждения v1 (jacobian_v1_identity_refuted), формализованная первопричина (det_expansion_single_pass: ветвь `det` одношагового резолвера не спускается в произведения) и исправленное утверждение с опуском разрешения в произведения детерминанта (Jacobian_singularity_resolved через A6-стадию канона). Использованы только ядро-нативные конструкции: индуктив, rfl, rw, injection, cases. Ни ℝ/ℚ/ℂ, ни Mathlib-тактик в теле нет; строка `import Mathlib` телом не используется и удаляется в производной. Фактическим основанием статуса будет прогон джобы kernel-check (A-0007: локальный тулчейн недоступен из песочницы).
  ...
  ============================================================================-/
#print axioms RICIS_Jacobian.det_expansion_single_pass
#print axioms RICIS_Jacobian.RExpr_mul_ne_zeroF
#print axioms RICIS_Jacobian.jacobian_v1_identity_refuted
#print axioms RICIS_Jacobian.A6_pair_reaches_geometric_measure
#print axioms RICIS_Jacobian.A6_geometric_realization
#print axioms RICIS_Jacobian.A10_mul_zero

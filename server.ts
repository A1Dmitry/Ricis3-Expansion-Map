...
7. СТРОГОЕ СОБЛЮДЕНИЕ ПРАВИЛ RICIS-III v7.9 (v7.9_A15_L1_aligned):
   - Запрет на круговую поруку (Petitio Principii): нельзя вычислять функции в неизвестных корнях (x = p), заявляя, что их вычисление находит p.
   - Разрешение сингулярностей равного порядка: выполняется строго через Аксиому A15 (Equal-Order Structural Profile Law): при ord_a(F) = ord_a(G) = d < \\infty вычисляется структурный профиль \\pi_d(f,a) = D^d f(a) / D^d g(a). Запрещено использовать классический непрерывный предел Лопиталя.
   - Полярная нормализация (SP5): тригонометрические комбинации a \\cos\\theta + b \\sin\\theta нормализуются в полярную форму r \\cos(\\theta - \\phi) до индексации SP4.
   - Аксиома A4 строго определяет отношение семантических индексов 0_F / 0_G = F / G.
   - Любые нерешенные задачи повышенной сложности NP строго маркируются как HYPOTHESIS.

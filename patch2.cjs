...
for (let i = 0; i < lines.length; i++) {
  ...
  for (const id of targetNodeIds) {
    if (line.includes(`id: '${id}'`) || line.includes(`id: "${id}"`) || line.includes(`"id": "${id}"`)) {
      for (let j = i + 1; j < i + 15; j++) {
        if (lines[j] && (lines[j].includes('unresolved') || lines[j].includes('partial')) && lines[j].includes('state')) {
          lines[j] = lines[j].replace(/"state":\s*"unresolved"/, `"state": "resolved"`);
          lines[j] = lines[j].replace(/"state":\s*"partial"/, `"state": "resolved"`);
          lines[j] = lines[j].replace(/'state':\s*'unresolved'/, `'state': 'resolved'`);
          lines[j] = lines[j].replace(/'state':\s*'partial'/, `'state': 'resolved'`);
          lines[j] = lines[j].replace(/state:\s*'unresolved'/, `state: 'resolved'`);
          lines[j] = lines[j].replace(/state:\s*'partial'/, `state: 'resolved'`);
...
console.log('Successfully patched initialMap.ts nodes again');

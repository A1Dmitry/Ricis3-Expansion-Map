...
* **Ядровая проверка пакета 2 (2026-09-14, run [34870620154](https://github.com/A1Dmitry/Ricis3-Expansion-Map/actions/runs/34870620154), 12 целей):** **8 целей exit 0 без `sorryAx`, 4 цели exit 1**. Сырое evidence: [`…34870620154.pr-comment.txt`](docs/05-evidence/proofs/lean-kernel-run-34870620154.pr-comment.txt); раздел §6 в [`lean-core-checks-run-2026-09-14.md`](docs/05-evidence/proofs/lean-core-checks-run-2026-09-14.md); реестр v2 (12 артефактов, 8 находок).
  * **Подстановка `ℕ → Nat` подтверждена:** `ricis-backend-exact-reduction` — exit 0, 22 теоремы (6 без аксиом, 16 × `propext`), `sorryAx` отсутствует → `LEAN_VERIFIED` (в первом прогоне было 22 ошибки и 8 × `sorryAx`). `database-a6-0_5_inf_3` и `database-registry-120-jacobian` — exit 0, по 19 теорем (16 без аксиом) → `LEAN_VERIFIED`.
  * **`ricis-v79-monolith`:** `sorryAx` устранён полностью (31 теорема напечатана: 3 без аксиом, 28 × `propext`), но exit 1 — `392:2 error: No goals to be solved` (8 избыточных буллетов `· rfl` после `repeat constructor`, который на ядровой базе закрывает все 9 конъюнктов сам). По критерию «нет ошибок компилятора» статус **не повышен** (F-06).
  * **`ricis-jacobian-conjecture`:** после переименования `partial → partialDeriv` файл парсится, но `62:2 error: Tactic `rfl` failed: resolveRICIS (F.zeroF.det RExpr.zero RExpr.zero G.infF) is not definitionally equal to (F.mul G).sub RExpr.zero.zeroF` → `Jacobian_singularity_resolved` зависит от `[propext, sorryAx]`. Тождество не определительно, `rfl` — ядровая тактика: `TRUSTED_AXIOM` необоснован (F-01, усилен).
  * **`ricis-kernel-ast-sp5`:** `ℚ → Rat` устранило нотацию, но `26:2 error(lean.synthInstanceFailed): failed to synthesize instance of type class Decidable (a = b)` → обе теоремы с `sorryAx` (F-07). Ремонт: `deriving DecidableEq` у `RExpr`.
  * **`ricis-seed-expansion-a11`:** заявленная замена Mathlib-леммы `List.mem_of_mem_append_left` → `List.mem_append.mpr (Or.inl hr)` неприменима: `104:4 Type mismatch` (после `induction` цель `r ∈ a✝¹.rules` не содержит `++`), `105:4 Tactic `split` failed`, `156:63`/`165:50`/`176:105 unsolved goals` → 3 из 6 теорем с `sorryAx` (F-08, дефект собственного ремонта).
...
* **Дефекты механизма верификации, найденные прогоном ядра (2026-09-14):** заявление «неисправленные дефекты отсутствуют» более не соответствует фактам и уточнено.
  ...
  * **F-06 (MEDIUM):** v79 — все 31 теорема доказаны без `sorryAx`, но exit 1 из-за избыточных тактических буллетов; статус не повышается до устранения.
  * **F-07 (HIGH):** SP5 — в ядре Lean 4.33.1 не синтезируется `Decidable (a = b)` для `RExpr`; обе теоремы с `sorryAx`.
  * **F-08 (MEDIUM):** A11 — подстановка, заявленная мной как ядровый эквивалент Mathlib-леммы, неприменима в точке 104; отказ собственного ремонта зафиксирован, а не замолчан.

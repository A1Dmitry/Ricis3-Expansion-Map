...
// The problematic block in JSX:
const oldBlock = code.substring(
  code.indexOf("{selectedNode && selectedNode.state !=="),
  code.indexOf("</button>", code.indexOf("{selectedNode && selectedNode.state !==")) + 9
) + "\n              )}";

const newBlock = `{selectedNode && selectedNode.state !== 'resolved' && isNodeAvailable(selectedNode, map) && (
...
if (code.includes("{selectedNode && selectedNode.state !==")) {
  code = code.replace(oldBlock, newBlock);
  fs.writeFileSync('src/ui/Map3D.tsx', code);
  console.log('Fixed JSX safely.');
}
